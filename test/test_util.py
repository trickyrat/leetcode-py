...
class TestUtil:
    ...
    def test_generate_list_node(
        ...
        head = self.util.generate_list_node(test_input)
        actual = self.util.list_node_to_string(head)
        assert expected == actual

    def test_generate_list_node_empty(self):
        nodes = []
        result = self.util.generate_list_node(nodes)
    ...
    def test_empty_list(self):
        head = None
        expected = ""
        assert self.util.list_node_to_string(head) == expected
    ...
    def test_list_node_to_string_circular_start_in_middle(self):
        ...
        head.next.next.next = head
        assert self.util.list_node_to_string(head) == "2->3->1->2"

    def test_list_node_to_string_non_circular_same_values(self):
        head = ListNode(1)
        head.next = ListNode(1)
        head.next.next = ListNode(1)
        assert self.util.list_node_to_string(head) == "1->1->1"
    ...
    def test_generate_tree_node(self, nums, expected):
        actual_root = self.util.generate_tree_node(nums)
    ...
    def test_generate_n_tree_node_empty_list(self):
        nums = []
        root = self.util.generate_n_tree_node(nums)
        assert root is None
